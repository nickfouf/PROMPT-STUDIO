...
[SYSTEM INSTRUCTIONS: RESPONSE FORMATTING]
You are operating under a strict block-based parsing protocol. Split your response into distinct blocks. Output all text and operations in a single response.

RULES:
1. Every part of your response MUST be wrapped in tags containing a unique identifier: uid="${uid}".
2. The closing tag MUST perfectly match the opening tag and include the uid. Example: </markdown uid="${uid}">.
3. NEVER use CDATA sections.
4. Paths must ALWAYS be relative to the project root.
5. Use <markdown uid="${uid}"> ... </markdown uid="${uid}"> for explanations.
6. Use <terminal_op uid="${uid}" type="cmd|powershell" path="relative/dir"> ... </terminal_op uid="${uid}"> for terminal commands.
7. Use <file_op uid="${uid}" action="update|create|delete" path="relative/path/to/file"> ... </file_op uid="${uid}"> for file changes.
...
`;
    ...
    parseResponse(responseText, uid) {
        ...
        while ((match = topLevelRegex.exec(responseText)) !== null) {
    ...
    /**
     * Smart overlap cleaner that maps lines to ignore empty string alignment issues.
     */
    _cleanOverlap(startText, endText) {
        let sLines = startText.split(/\r?\n/);
        let eLines = endText.split(/\r?\n/);

        // Extract non-empty trimmed lines with their original array indices
        let sValid = sLines.map((l, i) => ({text: l.trim(), idx: i})).filter(x => x.text);
        let eValid = eLines.map((l, i) => ({text: l.trim(), idx: i})).filter(x => x.text);

        let maxOverlap = Math.min(sValid.length, eValid.length);
        let overlapIdxInEnd = -1;

        for (let i = maxOverlap; i > 0; i--) {
            let tail = sValid.slice(-i).map(x => x.text).join('\n');
            let head = eValid.slice(0, i).map(x => x.text).join('\n');

            if (tail === head) {
                // We found the overlap! Get the raw array index of the last matched line
        ...
        if (overlapIdxInEnd !== -1) {
            // Slice the raw endText array right after the overlapping lines
    ...
    applyFileOp(basePath, block) {
        ...
        if (block.attributes.action === 'update') {
            ...
            // SCENARIO A: Standard <search> block
            if (block.operations.search) {
                let searchText = block.operations.search;

                if (fileContent.includes(searchText)) {
                    fs.writeFileSync(targetPath, fileContent.replace(searchText, replaceText), 'utf-8');
                    return true;
                }
                ...
                if (fuzzyStr) {
                    ...
                    if (regex.test(fileContent)) {
                        fs.writeFileSync(targetPath, fileContent.replace(regex, replaceText), 'utf-8');
                        return true;
                    }
                }
            ...
            // SCENARIO B: Large Block <search_start> and <search_end>
            if (block.operations.search_start && block.operations.search_end) {
                ...
                // Level 2/3: Fuzzy Regex Span Match
                ...
                if (startFuzzy && endFuzzy) {
                    ...
                    if (spanRegex.test(fileContent)) {
                        fs.writeFileSync(targetPath, fileContent.replace(spanRegex, replaceText), 'utf-8');
                        return true;
                    }
                }
