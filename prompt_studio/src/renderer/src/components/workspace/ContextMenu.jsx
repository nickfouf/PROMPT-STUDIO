...
import { Edit2, Scissors, Copy, Trash2, Clipboard, FolderMinus } from 'lucide-react';
...
export default function ContextMenu({ contextMenu, selectedIds, clipboard, onAction, isMac }) {
  ...
  return (
    ...
    >
      ...
      {contextMenu.isRoot && (
        <ContextMenuItem icon={<FolderMinus className="w-4 h-4 text-orange-400" />} label="Remove from Project" onClick={() => onAction('remove-root')} className="text-orange-400 hover:bg-orange-500/20 hover:text-orange-300" />
      )}
