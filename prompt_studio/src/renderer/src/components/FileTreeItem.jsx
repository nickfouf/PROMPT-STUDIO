...
import { Folder, FolderOpen, ChevronRight, ChevronDown, Eye, EyeOff, LayoutList, ArrowRight, Loader2, FolderMinus } from 'lucide-react';
...
const FileTreeItem = React.memo(function FileTreeItem({
  ...
  let intrinsicVis = visibilityMap[normPath] || defaultVis;
  ...
  let nextVis;
  if (!isText) {
    nextVis = intrinsicVis === 'outline' ? 'hidden' : 'outline';
  } else {
    nextVis = intrinsicVis === 'full' ? 'outline' : (intrinsicVis === 'outline' ? 'hidden' : 'full');
  }
  ...
  const handleDragStart = (e) => {
    if (renamingNodeId === node.id) { e.preventDefault(); return; }
  ...
  const handleDragEnter = (e) => {
    if (!isFolder) return;
    if (e.dataTransfer.types.includes('application/x-file-tree-ids')) {
      e.preventDefault(); setDragCounter(prev => prev + 1);
    }
  };
  ...
  const handleDrop = (e) => {
    ...
    if (data) {
      ...
      } catch (err) { console.error('Failed to parse drag payload', err); }
    }
  };
  ...
  return (
    <div className={isRoot ? "mb-1" : ""}>
      <div
        ...
        onDrop={handleDrop}
        className={` flex items-center py-1.5 px-2 rounded cursor-pointer text-sm transition-colors group select-none relative ${isSelected ? 'bg-blue-600/30 text-blue-100 border border-blue-500/50' : (isRoot ? 'bg-gray-800/80 hover:bg-gray-700 border border-transparent border-l-2 border-l-blue-500 text-gray-200' : 'hover:bg-gray-800 border border-transparent text-gray-400')} ${effectiveVis === 'hidden' ? 'opacity-40' : ''} ${effectiveVis === 'outline' ? 'text-gray-400 italic' : ''} ${highlightClass} ${isDragOver ? '!bg-blue-500/30 !border-blue-400 ring-1 ring-blue-400' : ''}  `}
        style={rowStyle}
        onClick={(e) => onSelect(e, node)}
        onContextMenu={(e) => onContextMenu && onContextMenu(e, node)}
        onDoubleClick={(e) => {
          if (isFolder) { e.stopPropagation(); onToggleExpand(node.id); }
        }}
      >
        <span
        ...
        {renamingNodeId === node.id ? (
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Escape') {
                e.target.value = node.name; e.target.blur();
              }
            }}
        ...
        {node._isScanning && (
          <Loader2 className="w-3.5 h-3.5 animate-spin text-blue-400 ml-2 shrink-0" title="Scanning folder..." />
        )}

        {isRoot && (
          <div className="ml-2 flex items-center space-x-1 shrink-0">
            {!node.isForeign ? (
               <span className="px-1.5 py-0.5 text-[9px] uppercase font-bold tracking-wider text-blue-300 bg-blue-900/50 rounded border border-blue-700/50 shadow-inner">
                 Main Project
               </span>
            ) : (
               <span className="px-1.5 py-0.5 text-[9px] uppercase font-bold tracking-wider text-orange-300 bg-orange-900/50 rounded border border-orange-700/50 shadow-inner">
                 Foreign
               </span>
            )}
            ...
            {node.isForeign && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (onRemoveRoot) onRemoveRoot(node);
                }}
                className="p-1 hover:bg-red-500/20 text-gray-500 hover:text-red-400 rounded transition-colors opacity-0 group-hover:opacity-100"
        ...
        <button
          onClick={(e) => {
            ...
            onToggleVisibility(node.id, nextVis, isCascade);
          }}
          className={` ml-2 rounded transition-all shrink-0 flex items-center justify-center ${isOverridden ? 'px-1.5 py-0.5 bg-gray-800/80 border border-gray-700/80 hover:bg-gray-700 opacity-100' : intrinsicVis === 'full' ? 'p-1 text-gray-500 hover:text-white hover:bg-gray-600 opacity-0 group-hover:opacity-100' : intrinsicVis === 'outline' ? 'p-1 text-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20 opacity-100' : 'p-1 text-red-400 bg-red-400/10 hover:bg-red-400/20 opacity-100'}  `}
          title={tooltipText}
        >
      ...
      {isFolder && isExpanded && isRendering && (
        <div className="flex items-center text-gray-500 text-xs py-1.5" style={{ paddingLeft: `${((level + 1) * 12) + 28}px` }}>
          <Loader2 className="w-3.5 h-3.5 animate-spin mr-2" />
          Loading contents...
        </div>
      )}

      {isFolder && renderChildren && node.children && (
